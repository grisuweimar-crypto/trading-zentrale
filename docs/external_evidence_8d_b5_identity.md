...
- the internal scanner/history determines that a symbol was observable by the project at a scanner-generation timestamp;
...
## Scanner time semantics

The productive scanner artifact carries both:

- `as_of`: a date-only market observation date;
- `generated_at`: the timezone-aware timestamp at which the scanner artifact was generated.

B5 uses `generated_at` as the scanner knowledge timestamp. The date-only `as_of` field is not used to decide whether external evidence was already knowable, because inventing an intraday clock time could create future leakage.

## Inputs

1. historical scanner observations containing exact `symbol` and timezone-aware `generated_at` by default;
2. one or more PIT-validated `external_evidence_8d_sec_insider_bulk_v1` evidence artifacts.

The CLI exposes `--asof-column` for an explicitly verified alternative timezone-aware knowledge timestamp, but defaults to `generated_at`.
...
For each issuer CIK, B5 considers only identity evidence with `valid_from <= scanner_generated_at` and determines the latest symbol that was known for that CIK at that time.
...
Date-only knowledge timestamps are rejected. B5 does not invent an intraday time because doing so could make a filing appear known before the scanner actually ran.
...
- `insider_feature_grid.csv` containing only deduplicated `(issuer_cik, as_of)` pairs from `VERIFIED_FAMILY_PIT_IDENTITY` rows. Here `as_of` is the timezone-aware scanner knowledge timestamp supplied to B5, normally `generated_at`.
...
- scanner knowledge time defaults to timezone-aware `generated_at`;
- date-only market `as_of` is not promoted to an intraday knowledge timestamp;
