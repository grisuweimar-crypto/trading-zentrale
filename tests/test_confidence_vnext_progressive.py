...
def _write_fixture(tmp_path, include_20t_outcomes: bool = False):
    claims, outcomes = _two_date_fixture(
        include_20t_outcomes=include_20t_outcomes
    )
...
def test_historical_cutoff_filters_late_peer_before_peer_baseline_derivation():
    ...
    outcomes = pd.DataFrame(
        [
            ...
            _outcome(
                ...
                "2026-10-09",
                0.50,
                evaluated_at="2026-10-10T16:00:00Z",
            ),
        ],
        columns=OUTCOME_COLUMNS_V2,
    )
    ...
    assert float(
        by_symbol.loc["AAA", "peer_median_return"]
    ) == pytest.approx(0.03)
    assert float(
        by_symbol.loc["BBB", "peer_median_return"]
    ) == pytest.approx(0.01)


def test_unspaced_late_peer_is_bound_into_retained_row_dependency():
    prior_forbidden = [
        "2026-09-25",
        "2026-09-28",
        "2026-09-29",
    ...
    prior_aaa = _claim(
        "AAA", "prior", "2026-10-01", 5, prior_forbidden
    )
    current_aaa = _claim(
        "AAA", "shared", "2026-10-02", 5, current_forbidden
    )
    current_bbb = _claim(
        "BBB", "shared", "2026-10-02", 5, current_forbidden
    )
    claims = pd.DataFrame(
        [prior_aaa, current_aaa, current_bbb],
    ...
    outcomes = pd.DataFrame(
        [
            _outcome(
                ...
                "2026-10-09",
                0.05,
                evaluated_at="2026-10-12T16:00:00Z",
            ),
            _outcome(
                current_bbb,
                "2026-10-09",
    ...
    assert list(frame["symbol"]) == ["BBB"]
    assert float(frame.iloc[0]["peer_median_return"]) == pytest.approx(
        0.05
    )
    assert pd.Timestamp(
        frame.iloc[0]["peer_dependency_evaluated_at"]
    ) == pd.Timestamp("2026-10-12T16:00:00Z")
    assert pd.Timestamp(
        frame.iloc[0]["evaluated_at"]
    ) == pd.Timestamp("2026-10-10T16:00:00Z")
...
def test_5t_can_start_learning_while_20t_is_still_collecting():
    ...
    twenty = progressive_training_frame(
        claims, outcomes, horizon=20
    )
...
def test_candidate_is_hash_chained_research_only_and_does_not_borrow_horizons():
    ...
    assert (
        candidate["semantics"][
            "training_cutoff_binds_peer_label_dependencies"
        ]
        is True
    )
    ...
    assert (
        candidate["evaluation"][
            "evaluation_claim_generated_at_must_be_after_training_cutoff"
        ]
        is True
    )


def test_candidate_cutoff_uses_later_peer_dependency_maturity():
    claims, outcomes = _two_date_fixture(include_20t_outcomes=False)
    five = progressive_training_frame(claims, outcomes, horizon=5)
    later_dependency = pd.Timestamp("2026-10-20T16:00:00Z")
    assert pd.to_datetime(
        five["evaluated_at"], utc=True
    ).max() < later_dependency
    five.loc[
        five.index[0], "peer_dependency_evaluated_at"
    ] = later_dependency

    candidate = build_candidate_version(five, horizon=5)

    assert candidate is not None
    assert pd.Timestamp(candidate["training_cutoff"]) == later_dependency
...
def test_progressive_runner_is_idempotent_for_unchanged_evidence(tmp_path):
    ...
    first = run_progressive_learning(
        claims_path,
        outcomes_path,
        versions_path,
        report_path,
    )
    second = run_progressive_learning(
        claims_path,
        outcomes_path,
        versions_path,
        report_path,
    ...
    assert (
        second["semantics"][
            "historical_cutoff_applied_before_peer_label_derivation"
        ]
        is True
    )
    assert (
...
def test_version_hash_detects_mutation(tmp_path):
    ...
    item = json.loads(
        versions_path.read_text(encoding="utf-8").splitlines()[0]
    )
