...
def claim_spacing_membership(claims: pd.DataFrame) -> set[str]:
    """Freeze five-session membership using claims only, never label availability."""
    ...
    work["_start"] = pd.to_datetime(
        work["start_market_date"], errors="coerce"
    ).dt.normalize()
    work["_generated"] = pd.to_datetime(
        work["generated_at"], errors="coerce", utc=True
    )
    work = work.loc[
        work["_start"].notna() & work["_generated"].notna()
    ].copy()

    keep: set[str] = set()
    for (_, _), group in work.groupby(
        ["horizon_sessions", "symbol"], sort=False
    ):
        ...
        for _, row in ordered.iterrows():
            ...
            forbidden = _forbidden_dates(
                row.get("cooldown_forbidden_start_dates")
            )
...
def _peer_dependency_table(
    claims: pd.DataFrame,
    outcomes: pd.DataFrame,
) -> pd.DataFrame:
    """Latest maturity timestamp of each peer-label cohort actually available.

    ``derive_peer_labels_v2`` derives peer baselines inside the exact
    ``(snapshot_id, horizon_sessions)`` outcome cohort.  Spacing is deliberately
    applied only afterwards, so an outcome from an unspaced claim can still be
    an input to a retained row's peer label.  The dependency timestamp below
    makes that indirect information dependency explicit and auditable.
    """
    ...
    dependencies = outcomes[
        ["claim_id", "horizon_sessions", "evaluated_at"]
    ].merge(
        claims[["claim_id", "snapshot_id"]],
        on="claim_id",
        how="inner",
        validate="one_to_one",
    )
    dependencies["_horizon_key"] = pd.to_numeric(
        dependencies["horizon_sessions"], errors="coerce"
    ).astype("Int64")
    dependencies["peer_dependency_evaluated_at"] = pd.to_datetime(
        dependencies["evaluated_at"], errors="coerce", utc=True
    )
    if (
        dependencies["_horizon_key"].isna().any()
        or dependencies["peer_dependency_evaluated_at"].isna().any()
    ):
        raise ValueError("Phase 5C peer dependency chronology failure")

    return (
        dependencies.groupby(
            ["snapshot_id", "_horizon_key"], as_index=False
        )["peer_dependency_evaluated_at"]
        .max()
        .sort_values(
            ["snapshot_id", "_horizon_key"], kind="mergesort"
        )
...
def progressive_training_frame(
    ...
    *,
    ...
    """Return claim-spaced, fully matured evidence knowable by the cutoff.

    Historical cutoffs are applied before peer-label derivation.  Every retained
    row also carries the latest maturity timestamp of the complete peer cohort
    used to derive its peer baseline, including peers later excluded by spacing.
    """
    ...
    if knowable_by is not None:
        ...
        known_outcomes = known_outcomes.loc[
            outcome_known
            & known_outcomes["claim_id"].astype(str).isin(known_ids)
        ].copy()
    ...
    peer_dependencies = _peer_dependency_table(
        known_claims, known_outcomes
    )
    derived = derive_peer_labels_v2(known_claims, known_outcomes)
    if derived.empty:
        return derived

    availability = known_outcomes[
        ["claim_id", "evaluated_at", "end_market_date"]
    ].copy()
    frame = derived.merge(
        availability,
        on="claim_id",
    ...
    frame["_horizon_key"] = pd.to_numeric(
        frame["horizon_sessions"], errors="coerce"
    ).astype("Int64")
    if frame["_horizon_key"].isna().any():
        raise ValueError("Phase 5C training horizon provenance failure")
    frame = frame.merge(
        peer_dependencies,
        on=["snapshot_id", "_horizon_key"],
        how="left",
        validate="many_to_one",
    )
    if frame["peer_dependency_evaluated_at"].isna().any():
        raise ValueError("Phase 5C peer dependency binding failure")

    frame = frame.loc[
        frame["claim_id"].astype(str).isin(membership)
        & frame["_horizon_key"].eq(int(horizon))
    ].copy()
    frame["_evaluated_at"] = pd.to_datetime(
        frame["evaluated_at"], errors="coerce", utc=True
    )
    if frame["_evaluated_at"].isna().any():
        raise ValueError("Phase 5C retained outcome chronology failure")

    return (
        frame.sort_values(
            ["_evaluated_at", "as_of", "symbol", "claim_id"],
            kind="mergesort",
        )
        .drop(columns=["_evaluated_at", "_horizon_key"])
        .reset_index(drop=True)
    )
...
def _state_summary(
    ...
    directional = frame.loc[
        _numeric(frame, "direction_hit").notna()
    ].copy()
    signed = _numeric(
        directional, "signed_peer_excess"
    ).dropna()
    ...
    return {
        ...
        "observation_dates": int(
            pd.to_datetime(frame["as_of"], errors="coerce").nunique()
        )
        if len(frame)
        else 0,
        "support_regions": int(_support_regions(frame, horizon)),
        "direction_hit_rate": float(hit.mean()) if len(hit) else None,
        "mean_signed_peer_excess": float(signed.mean())
        if len(signed)
        else None,
        "median_signed_peer_excess": float(signed.median())
        if len(signed)
        else None,
        "mean_adverse_excursion": float(adverse.mean())
        if len(adverse)
        else None,
        "mean_path_max_drawdown": float(drawdown.mean())
        if len(drawdown)
        else None,
    }


def _single_state_table(
    frame: pd.DataFrame,
    field: str,
    horizon: int,
) -> dict[str, object]:
    ...
    return {
        state: _state_summary(
            frame.loc[values.eq(state)].copy(), horizon
        )
        for state in sorted(values.unique())
    }


def _multi_state_table(
    frame: pd.DataFrame,
    field: str,
    horizon: int,
) -> dict[str, object]:
    ...
    for row in frame.to_dict("records"):
        ...
        states = sorted(
            set(value for value in raw.split("|") if value)
        ) or ["missing"]
        for state in states:
            item = dict(row)
            item["_state"] = state
    ...
    return {
        state: _state_summary(
            exploded.loc[
                exploded["_state"].eq(state)
            ].copy(),
            horizon,
        )
        for state in sorted(
            exploded["_state"].astype(str).unique()
        )
    }
...
def horizon_readiness(
    ...
    observation_dates = (
        int(pd.to_datetime(frame["as_of"], errors="coerce").nunique())
        if not frame.empty
        else 0
    )
    symbols = (
        int(frame["symbol"].astype(str).nunique())
        if not frame.empty
        else 0
    )
    directional_symbols = (
        int(directional["symbol"].astype(str).nunique())
    ...
    robust_base = (
        pilot_ready
        and support_regions >= MIN_ROBUST_SUPPORT_REGIONS
    )
...
def _training_fingerprint(frame: pd.DataFrame) -> str:
    if frame.empty:
        empty = pd.DataFrame(
            columns=TRAINING_FINGERPRINT_COLUMNS
        )
        return evidence_fingerprint(empty)
    missing = [
        column
        for column in TRAINING_FINGERPRINT_COLUMNS
        if column not in frame.columns
    ]
    if missing:
        raise ValueError(
            f"Phase 5C training columns missing: {missing}"
        )
    return evidence_fingerprint(
        frame.loc[:, TRAINING_FINGERPRINT_COLUMNS].copy()
    )
...
def _version_sha256(
    version_without_hash: dict[str, object],
) -> str:
    return sha256(
        _canonical_json(version_without_hash).encode("utf-8")
    ).hexdigest()
...
def build_candidate_version(
    ...
    *,
    ...
    peer_dependencies = pd.to_datetime(
        frame["peer_dependency_evaluated_at"],
        errors="coerce",
        utc=True,
    )
    as_of = pd.to_datetime(
        frame["as_of"], errors="coerce", utc=True
    )
    end_dates = pd.to_datetime(
        frame["end_market_date"], errors="coerce"
    )
    if (
        evaluated.empty
        or peer_dependencies.empty
        or as_of.empty
        or end_dates.empty
        or evaluated.isna().any()
        or peer_dependencies.isna().any()
        or as_of.isna().any()
        or end_dates.isna().any()
    ):
        raise ValueError(
            "Phase 5C candidate requires valid evidence chronology"
        )
    training_cutoff = max(
        evaluated.max(), peer_dependencies.max()
    )
    ...
    version: dict[str, object] = {
        ...
        "training_rows": int(len(frame)),
        "readiness": readiness,
        "learned_state_reliability": _learned_tables(
            frame, horizon
        ),
        "evaluation": {
        ...
        "semantics": {
            ...
            "training_cutoff_binds_peer_label_dependencies": True,
...
def _validate_versions(
    ...
    for line_no, item in enumerate(versions, start=1):
        if item.get("schema_version") != SCHEMA_VERSION:
            raise ValueError(
                f"unexpected Phase 5C version schema at line {line_no}"
            )
        supplied_hash = str(item.get("version_sha256") or "")
        if len(supplied_hash) != 64:
            raise ValueError(
                f"invalid Phase 5C version hash at line {line_no}"
            )
        payload = dict(item)
        payload.pop("version_sha256", None)
        if supplied_hash != _version_sha256(payload):
            raise ValueError(
                f"Phase 5C version integrity failure at line {line_no}"
            )
        if (
            str(item.get("previous_version_sha256") or "")
            != expected_previous
        ):
            raise ValueError(
                f"Phase 5C version chain failure at line {line_no}"
            )

        horizon = int(item.get("horizon_sessions", -1))
        fingerprint = str(
            item.get("evidence_fingerprint") or ""
        )
        expected_id = _version_id(horizon, fingerprint)
        version_id = str(item.get("version_id") or "")
        if horizon not in HORIZONS or version_id != expected_id:
            raise ValueError(
                "Phase 5C deterministic version identity failure "
                f"at line {line_no}"
            )
    ...
    if len(ids) != len(set(ids)):
        raise ValueError(
            "Phase 5C model-version archive is not unique"
        )
...
def _read_versions(
    ...
    for line_no, line in enumerate(
        ...
        try:
            ...
            raise ValueError(
                f"invalid Phase 5C version JSON at line {line_no}"
            ) from exc
...
def _validate_previous_report_anchor(
    ...
    if previous.get("schema_version") != SCHEMA_VERSION:
        raise ValueError(
            "unexpected previous Phase 5C report schema"
        )
    ...
    if prior_count < 0 or len(versions) < prior_count:
        raise ValueError(
            "Phase 5C model-version archive was truncated"
        )
    if prior_count == 0:
        if prior_tip:
            raise ValueError(
                "Phase 5C zero-version report has a non-empty chain tip"
            )
        return
    observed_tip = str(
        versions[prior_count - 1].get("version_sha256") or ""
    )
    if observed_tip != prior_tip:
        raise ValueError(
            "Phase 5C historical model-version prefix changed"
        )
...
def run_progressive_learning(
    ...
    for horizon in HORIZONS:
        frame = progressive_training_frame(
            claims, outcomes, horizon=horizon
        )
        readiness = horizon_readiness(frame, horizon)
        previous_hash = (
            str(versions[-1]["version_sha256"])
            if versions
            else ""
        )
    ...
    result = {
        ...
        "schema_version": SCHEMA_VERSION,
        "status": (
            "progressive_learning_active"
            if any(
                item["pilot_ready"] for item in horizons.values()
            )
