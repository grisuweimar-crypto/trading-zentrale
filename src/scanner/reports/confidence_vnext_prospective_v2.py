...
This module starts a new prospective v2 stream.  It never rewrites or
backfills Phase-4E shadow-v1.  V2 integrity-binds the claim-time Statistical
Context and the exact market-session path used to mature each forward outcome.
...
def _latest_stock_context(history: pd.DataFrame, latest: pd.DataFrame, latest_date: pd.Timestamp) -> pd.DataFrame:
    ...
    feature_columns = [column for column in feature_current.columns if column not in current.columns or column in {"date", "symbol"}]
...
def _claim_start_context(prices: pd.DataFrame, symbol: str, as_of: str) -> tuple[str, float | None, str, str, str, bool]:
    ...
    recent = [
        ...
        for value in group.iloc[max(0, pos - (FIXED_EVENT_SPACING_SESSIONS - 1)) : pos + 1]["_date"].tolist()
    ]
...
def _timing_context(feature_row: pd.Series, registry: dict[str, dict[str, object]]) -> tuple[list[dict[str, object]], dict[str, int]]:
    ...
    for pattern_name, raw_evidence in sorted(registry.items()):
        ...
        prerequisites = sorted({item for condition in conditions for item in _atom_prerequisites(condition)})
        missing = [item for item in prerequisites if item not in feature_row.index or not _present(feature_row.get(item))]
        if missing:
...
def _risk_context(feature_row: pd.Series, registry: dict[str, dict[str, object]]) -> list[dict[str, object]]:
...
def _return_claim_direction(row: dict[str, object]) -> str:
    ...
    if isinstance(selection, dict) and selection.get("state") == "robust" and selection.get("direction") in {"positive", "negative"}:
        directions.append(str(selection["direction"]))
    if isinstance(timing, dict) and timing.get("state") == "robust_claim" and timing.get("direction") in {"positive", "negative"}:
        directions.append(str(timing["direction"]))
    return next(iter(set(directions))) if len(set(directions)) == 1 else ""
...
def build_claim_rows_v2(
    ...
    *,
    ...
    feature_by_symbol = {str(row["symbol"]): pd.Series(row) for row in feature_frame.to_dict("records")}
    currency_by_symbol = {
        str(row["symbol"]): (_norm_currency(row.get("currency")) or "")
        for row in feature_frame.to_dict("records")
    }
    starts = {symbol: _claim_start_context(claim_prices, symbol, as_of) for symbol in feature_by_symbol}
    ...
    for row in current.get("rows") or []:
        ...
        timing_context, timing_unevaluable = _timing_context(feature_row, h_registry.get("timing") or {})
        risk_context = _risk_context(feature_row, h_registry.get("risk") or {})
        ...
        timing_states = sorted({str(item.get("state") or "unavailable") for item in timing_context})
        risk_states = sorted(
            f"{item.get('feature')}:{item.get('state') or 'unavailable'}:{item.get('claim_level') or 'unavailable'}"
            for item in risk_context
        )
        start_date, start_value, eligibility, unavailable_reason, cooldown_dates, cooldown_complete = starts[symbol]
        ...
        payload: dict[str, object] = {
            ...
            **fingerprints,
            ...
            "agreement_conflicts": "|".join(sorted(str(value) for value in (agreement.get("conflicts") or []))),
            "return_claim_direction": _return_claim_direction(row),
            "dq_selection_state": str(((dq.get("selection") or {}).get("state")) or "proxy_insufficient"),
            "dq_timing_state": str(((dq.get("timing") or {}).get("state")) or "proxy_insufficient"),
            "dq_risk_state": str(((dq.get("risk") or {}).get("state")) or "proxy_insufficient"),
            "volatility_application_status": volatility_status,
        }
        payload["claim_id"] = _sha_payload(payload)
        rows.append(payload)

    return pd.DataFrame(rows, columns=CLAIM_COLUMNS_V2).sort_values(
        ["symbol", "horizon_sessions"], kind="mergesort"
...
def append_claims_v2(existing: pd.DataFrame, new: pd.DataFrame) -> pd.DataFrame:
    ...
    old = existing.copy()
    fresh = new.copy()
    old_key = old["snapshot_id"].astype(str) + "|" + old["symbol"].astype(str) + "|" + old["horizon_sessions"].astype(str)
    fresh_key = fresh["snapshot_id"].astype(str) + "|" + fresh["symbol"].astype(str) + "|" + fresh["horizon_sessions"].astype(str)
...
def _mature_one_v2(claim: dict[str, object], group: pd.DataFrame, evaluated_at: str) -> dict[str, object] | None:
    ...
    target_utc = target_date.tz_localize("UTC") if target_date.tzinfo is None else target_date.tz_convert("UTC")
    ...
    if len(path) != horizon + 1 or path_dates.isna().any() or not np.isfinite(path).all() or (path <= 0).any():
        return None
    date_strings = [pd.Timestamp(value).normalize().date().isoformat() for value in path_dates]
    path_payload = [
    ...
    if payload["elapsed_market_sessions"] != horizon or payload["path_session_count"] != horizon + 1:
...
def compute_mature_outcomes_v2(
    ...
    for claim in claims.to_dict("records"):
        ...
        if matured is not None:
            rows.append(matured)
    return pd.DataFrame(rows, columns=OUTCOME_COLUMNS_V2) if rows else pd.DataFrame(columns=OUTCOME_COLUMNS_V2)
...
def append_outcomes_v2(existing: pd.DataFrame, new: pd.DataFrame) -> pd.DataFrame:
    ...
    if new.empty:
        return existing.copy().reset_index(drop=True)
    previous = dict(zip(existing["claim_id"].astype(str), existing["outcome_id"].astype(str)))
    keep: list[int] = []
...
def validate_v2_archives(claims: pd.DataFrame, outcomes: pd.DataFrame) -> None:
    ...
    if not outcomes.empty:
        if outcomes["claim_id"].astype(str).duplicated().any() or outcomes["outcome_id"].astype(str).duplicated().any():
        ...
        for row in outcomes.to_dict("records"):
            ...
            if horizon not in HORIZONS or int(row["elapsed_market_sessions"]) != horizon or int(row["path_session_count"]) != horizon + 1:
                raise ValueError("v2 outcome horizon provenance failure")


def derive_peer_labels_v2(claims: pd.DataFrame, outcomes: pd.DataFrame) -> pd.DataFrame:
    if claims.empty or outcomes.empty:
        return pd.DataFrame(columns=DERIVED_COLUMNS_V2)
    validate_v2_archives(claims, outcomes)
    claim_columns = [
        "claim_id", "snapshot_id", "return_claim_direction", "selection_statistical_state",
        "timing_matched_states", "risk_evidence_states", "agreement_state", "timing_model_state",
        "risk_model_state", "dq_selection_state", "dq_timing_state", "dq_risk_state",
        "cooldown_forbidden_start_dates", "cooldown_context_complete",
    ]
    work = outcomes.merge(claims[claim_columns], on="claim_id", how="inner", validate="one_to_one")
    ...
    for snapshot_id in work["snapshot_id"].astype(str).unique():
        ...
        for horizon in HORIZONS:
            ...
            work.loc[subset.index, "peer_excess"] = subset["return"].to_numpy(dtype=float) - peer.to_numpy(dtype=float)
...
def run_v2(
    ...
    *,
    ...
    new_claims = build_claim_rows_v2(
        phase4_report, history, latest, metadata, fingerprints, claim_prices, source_commit=source_commit
    )
    ...
    new_outcomes = compute_mature_outcomes_v2(claims, prices, existing_outcomes, evaluated_at)
