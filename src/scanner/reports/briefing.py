...
def build_briefing_from_csv(csv_path: Path, *, top_n: int = 3, language: str = "de") -> dict[str, Any]:
    ...
    c_perf = _first_col(df, ["perf_pct", "Perf %", "Perf%", "perf"])
    ...
    perf_window = "unknown"
    if c_perf:
        n = c_perf.strip().lower()
        if n in {"perf_pct", "perf %", "perf%", "perf"}:
            perf_window = "snapshot_to_snapshot"

    def _anomaly_for_perf(perf_f: float | None) -> list[dict[str, Any]]:
        ...
        if ap > 80:
            sev = "alert"
            lbl = "Sehr auffaellige Perf %"
            exp = f"Sehr hoher Wert. Pruefe Datenbasis/Zeitraum ({perf_window}) sowie Corporate Actions."
        elif ap > 30:
            sev = "warn"
            lbl = "Ungewoehnliche Perf %"
            exp = f"Erhoehter Wert. Pruefe Definition/Zeitraum ({perf_window}) und Datenkonsistenz."
        else:
            return out
        out.append(
            {
    ...
    for rank, (_, r) in enumerate(top_df.iterrows(), start=1):
        ...
        anomalies = _anomaly_for_perf(perf_f)
        if anomalies:
        ...
        if anomalies:
            next_checks[0] = {"id": "check_perf_basis", "text": f"Datenbasis von Perf % pruefen ({perf_window}, Referenzwerte, Zeitfenster)."}
    ...
    meta = {
        ...
        "metrics_basis": {
            "perf_pct": {
                "label": c_perf or "perf_pct",
                "window": perf_window,
                "definition_hint": "Change vs reference in loaded CSV; verify source window if values look extreme.",
            }
        },
    ...
    summary = {
        "what_to_do_next": [
            ...
            "Bei extremer Perf % zuerst Datenbasis und Zeitraum verifizieren.",
        ],
    ...
    diagnostics = {
        "data_quality": {
            ...
            "suspicious_values": [{"field": "perf_pct", "rule": "abs(perf_pct) > 30", "count": int(extreme_perf_count)}],
        },
        "build_info": {
