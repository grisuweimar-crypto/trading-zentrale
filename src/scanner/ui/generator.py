...
.moversItem .sym { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: clamp(68px, 24vw, 110px); }
.moversLeft { display:flex; align-items:center; gap: 6px; min-width: 0; }
...
.moversHead { display:flex; align-items:center; justify-content: space-between; gap: 8px; margin-bottom: 8px; }
.moversMiniBtn {
...
.moversMiniBtn:disabled { opacity: .45; cursor: default; }
.moversMiniBtn:not(:disabled):hover { border-color: rgba(96,165,250,.55); color: #dbeafe; }
.moversTag {
  appearance: none; -webkit-appearance: none;
  ...
  line-height: 1.2;
  cursor: pointer;
  max-width: clamp(72px, 22vw, 130px);
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.moversTag:hover { border-color: rgba(96,165,250,.55); color: #dbeafe; }
...
<div class="marketPanel" id="marketPanel" title="Marktumfeld aus deinem aktuellen Universe - nur Kontext, kein Score-Einfluss.">
  ...
  <div id="marketBody" class="marketGrid">
    ...
    <div class="marketCard" id="moversCard">
      <div class="moversHead">
        <div class="marketCardTitle" title="Staerkste Auf- und Abbewegungen nach Tagesbewegung (1D) im aktuellen Universe.">Movers</div>
        <button type="button" class="moversMiniBtn" id="moversClearCluster" title="Cluster/Sektor-Filter loesen">Cluster-Filter loesen</button>
      </div>
...
const btnMoversClearCluster = document.getElementById('moversClearCluster');
...
function renderMovers(rows) {
  ...
  if (btnMoversClearCluster) {
    const active = !!(clusterPick || '').toString().trim();
    btnMoversClearCluster.disabled = !active;
    btnMoversClearCluster.textContent = active ? `Cluster: ${clusterPick} x` : 'Cluster-Filter loesen';
  }

  function moverToneStyle(v) {
    if (v === null || v === undefined || !Number.isFinite(v)) return 'color: var(--muted);';
    const abs = Math.min(Math.abs(v), 8.0);
    const a = 0.55 + (abs / 8.0) * 0.45;
    if (v > 0) return `color: rgba(52, 211, 153, ${a.toFixed(3)}); text-shadow: 0 0 10px rgba(52,211,153,.18);`;
    if (v < 0) return `color: rgba(251, 113, 133, ${a.toFixed(3)}); text-shadow: 0 0 10px rgba(251,113,133,.14);`;
    return 'color: var(--muted);';
  }

  function applyClusterFromBadge(cl) {
    const cluster = (cl || '').toString().trim();
    if (!cluster) return;
    clusterPick = cluster;
    if (elClusterSel) {
      const has = Array.from(elClusterSel.options || []).some(o => (o.value || '') === cluster);
      if (!has) {
        const opt = document.createElement('option');
        opt.value = cluster;
  ...
  function itemHtml(x) {
    ...
    const cl = clusterLabel(r);
    const tag = cl ? `<button type="button" class="moversTag" data-cluster="${esc(cl)}" title="Filter auf ${esc(cl)} setzen">${esc(cl)}</button>` : '';
    ...
    const tone = moverToneStyle(x.p1d);
    return `<div class="moversItem"><span class="moversLeft">${s}${tag}<span class="moversMeta">${esc(y1)}</span></span><span class="val ${cls}" style="${tone}">${esc(d1)}</span></div>`;
  }
  ...
  for (const root of [elMoversUp, elMoversDown]) {
    root.querySelectorAll('button.moversTag[data-cluster]').forEach(btn => {
      btn.addEventListener('click', (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        applyClusterFromBadge(btn.getAttribute('data-cluster') || '');
      });
    });
  }
}
...
function renderMarketContext(rows, presetRows) {
  ...
  renderHeatmap(rows);
    ...
    if (btnMoversClearCluster) {
      btnMoversClearCluster.addEventListener('click', () => {
        clusterPick = '';
        if (elClusterSel) elClusterSel.value = '';
        refresh();
        saveState();
