...
DEFAULT_MARKET_PROXIES = Path("configs/external_evidence_8f_market_proxies_v1.json")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "series-catalog, market-proxy and exposure-map foundation."
        )
    )
    ...
    proxies = validate_market_proxies(
        market_proxies,
        allowed_factor_ids=allowed_factor_ids,
    )

    result = {
        "schema_version": "external_evidence_8f_foundation_gate_v2",
        "phase": "8F_macro_exposure_context",
        "status": "PASS_FOUNDATION_SOURCE_SERIES_AND_PROXY_CONTRACTS",
